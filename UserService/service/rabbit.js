...
class RabbitMQService {
    ...
    async publishUserEvent(event, userData) {
        const exchange = 'user_events';
        const routingKey = event; // e.g., 'user.created', 'user.updated'
        await this.publishMessage(exchange, routingKey, userData);
    }

    async subscribeToUserEvents(queueName, handler) {
        const exchange = 'user_events';
        const routingKey = 'user.*'; // Subscribe to all user events
