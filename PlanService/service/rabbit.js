...
class RabbitMQService {
    ...
    async publishPlanEvent(event, planData) {
        const exchange = 'plan_events';
        const routingKey = event; // e.g., 'plan.created', 'plan.updated'
        await this.publishMessage(exchange, routingKey, planData);
    }

    async subscribeToPlanEvents(queueName, handler) {
        const exchange = 'plan_events';
        const routingKey = 'plan.*'; // Subscribe to all plan events
